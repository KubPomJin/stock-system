...
> สถานะ: โปรแกรมนี้ **สร้างเสร็จและใช้งานจริงแล้ว** (ปัจจุบัน v1.6.2) — ไฟล์นี้อธิบาย
...
│   │       ├── sales.ts           # ขายหน้าร้าน: คีย์บิล, ปิดยอดประจำวัน, ตรวจโอน, สรุปยอดขาย
...
│   │                              #   sales, cashup, transfers, salesreport (+ salesCommon)
...
`zones`, `product_zones`, `order_docs(+ _lines)`, `order_counters`, `cash_closes`
...
| คีย์บิลขาย | staff | คีย์ใบสั่งสินค้าที่เขียนมือ (เลขเดียวกับกระดาษ) + รายการ + วิธีชำระ เงินสด/โอน/เงินสด+โอน/เครดิต — **ไม่ตัดสต๊อก** |
| ปิดยอดประจำวัน | staff | นับแบงก์/เหรียญ เทียบยอดที่ควรมี → ขาด/เกิน, เลขที่ใบที่ขาดหาย, พิมพ์ใบสรุป A4 · เจ้าของ (admin) กดตรวจ = ล็อกบิลของวันนั้น |
| ตรวจยอดโอน | stock_manager | ติ๊กบิลโอนที่เห็นยอดในบัญชีแล้ว ค้างข้ามวันได้ (ตรวจได้แม้วันถูกล็อก) |
| สรุปยอดขาย | stock_manager | ยอดรายเดือนทั้งปี → สินค้าขายดีของเดือน → แนวโน้มรายเดือนของสินค้า |
...
- **ขายหน้าร้าน (v1.6.0) = เก็บเงิน + ประวัติการขาย ไม่ตัดสต๊อก** — เจ้าของเลือกเองสำหรับช่วงทดลอง
  (หน้าร้านยังเขียนบิลมือ คีย์ทีหลัง ถ้าตัดสต๊อกแล้วคีย์ผิด ยอดสต๊อกจะเพี้ยนตาม)
  สต๊อกยังปรับจากการตรวจนับท้ายวัน · **ห้ามเพิ่ม stock_movements จาก `sales.ts` โดยไม่ถามก่อน**
  · บิลขาย **คือแถวใน `order_docs`** ใช้เลขที่เดียวกับใบกระดาษ (`A69-0012`) และ counter เดียวกับหน้าใบสั่งสินค้า
    (`syncCounterWithUsedNumber()` ใน `orders.ts`)
  · ยอดเงินสด/โอน/เครดิตของแต่ละบิล **คำนวณใน SQL จาก `payment_method` + `grand_total`**
    (ค่าคงที่ `CASH`/`TRANSFER`/`CREDIT` บนสุดของ `sales.ts`) ไม่เก็บเป็นคอลัมน์ — กันตัวเลขไม่ตรงบิล
  · `MIXED` = เงินสด + โอนในบิลเดียว (`transfer_amount` = ส่วนที่โอน)
  · **บิล "ไม่มีใบ"** (v1.6.1, `no_ticket = 1`) = ขายโดยไม่ได้ใช้ใบที่พิมพ์เลขไว้ — ระบบออกเลขชุด `N69-0001` เอง
    ไม่แตะ counter ของเล่ม A–D ไม่เข้าการหาเลขที่ขาดหาย · บิลกระดาษห้ามใช้เลขขึ้นต้นด้วย N
  · `transfer_status` เป็น NULL บนบิลที่มีโอน = ยังไม่ตรวจ (PENDING) · แก้ยอดโอนของบิลที่ตรวจแล้ว → กลับเป็น PENDING
  · **เจ้าของกดตรวจวันไหน = ล็อกบิลของวันนั้น** (เพิ่ม/แก้/ยกเลิก/คืนสถานะไม่ได้) จนกว่าจะกดยกเลิกการตรวจ
    แต่ **การตรวจยอดโอนทำได้เสมอ** เพราะเช็กบัญชีธนาคารทีหลังหลายวัน
  · `cash_closes` เก็บ snapshot ตัวเลขตอนปิดยอด — ถ้าบิลเปลี่ยนภายหลังหน้าจอจะขึ้นว่า "ปิดยอดใหม่"
  · ใบสรุป A4 อยู่ใน `#cashup-print-area` (ลูกตรงของ `<body>`) — **ห้ามย้ายเข้าไปใน modal ที่เป็น position:fixed**
    เครื่องพิมพ์เริ่มต้นของใบนี้เป็น PDF writer เพราะ LQ-310 ไม่มี A4
